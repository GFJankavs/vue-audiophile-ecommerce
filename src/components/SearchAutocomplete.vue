<template>
    <div class="autocomplete">
        <input type="text" :placeholder="placeholder" @input="onChange" v-model="search"
            @keydown.down="(e) => onArrowDown(e)" @keydown.up="onArrowUp" @keydown.enter="(e) => onEnter(e)"
            :class="inputClass" @keydown.escape="onEscape" />
        <ul id="autocomplete-results" v-show="isOpen" class="autocomplete-results" :style="{
        ...
        onArrowDown(e: Event) {
            ...
            if (isFocused && this.arrowCounter < this.results.length - 1) {
                ...
                if (focus < this.results.length) {
                    const results = document.querySelectorAll('.autocomplete-result') as NodeListOf<HTMLLIElement>;
                    results[focus].focus();
